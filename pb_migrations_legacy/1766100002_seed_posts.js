...
migrate((app) => {
    ...
    posts.forEach((post) => {
        try {
            ...
            // We use findRecordsByFilter with limit 1
            let existing;
            try {
                existing = app.findRecordsByFilter(collection, `slug = "${post.slug}"`, '', 1);
            } catch (e) {
                existing = [];
            }
            ...
            console.log("Error processing post: " + post.slug + " - " + e);
        }
    });
