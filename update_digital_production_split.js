...
async function updateServices() {
    ...
    try {
        ...
        // Update Producers Service
        try {
            const producers = await pb.collection('services').getFirstListItem('slug="digital-production"');
            console.log(`Updating Producers service: ${producers.slug}`);
            await pb.collection('services').update(producers.id, {
                sections: JSON.stringify(producersSections)
            });
            console.log("✅ Producers service updated!");
        } catch (e) {
            console.error("❌ Producers service 'digital-production' not found.");
        }

        // Update Companies Service
        try {
            const companies = await pb.collection('services').getFirstListItem('slug="digital-corporate"');
            console.log(`Updating Companies service: ${companies.slug}`);
            await pb.collection('services').update(companies.id, {
                sections: JSON.stringify(companiesSections)
            });
            console.log("✅ Companies service updated!");
        } catch (e) {
            console.error("❌ Companies service 'digital-corporate' not found.");
        }
