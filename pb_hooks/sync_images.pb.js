...
routerAdd("GET", "/api/dev/sync-images", (c) => {
    try {
        ...
        // imageMap constructed from migration files
        const imageMap = {
        ...
        /*
            ...
            "arri-amira": "https://www.bhphotovideo.com/cdn-cgi/image/fit=scale-down,width=500,quality=95/https://www.bhphotovideo.com/images/images500x500/panasonic_au_v35lt1g_cinema_varicam_lt_4k_1455802590_1226386.jpg", // Note: Using URL from migration file even if it looks like Varicam
            "panasonic-varicam-lt": "https://www.bhphotovideo.com/cdn-cgi/image/fit=scale-down,width=500,quality=95/https://www.bhphotovideo.com/images/images500x500/arri_k0_0014798_amira_camera_set_with_1513877808_1346962.jpg", // Using URL from migration file
        ...
        */
        ...
        logs.push(`Found ${records.length} records to process. Entering loop...`);
        ...
        for (const record of records) {
            ...
            if (logs.length < 10) {
                logs.push(`Record: ${slug}, Map Match: ${!!imageUrl}`);
            }

            if (!imageUrl) {
                skipped++;
                // logs.push(`Skipped: ${slug} (No URL in map)`);
                continue;
            }

            try {
                // FORCE UPDATE - Ignore existing image for now to ensure quality
                /*
                ...
                */
                ...
                if (res.statusCode === 200) {
                    ...
                    else if (lowerUrl.includes(".jpeg")) ext = ".jpg";
                    else if (lowerUrl.includes(".gif")) ext = ".gif";

                    const fileName = `${record.id}_${new Date().getTime()}${ext}`;
                    ...
                    updated++;
                    // logs.push(`Saved: ${slug}`);
                } else {
                    failed++;
                    logs.push(`Failed status ${res.statusCode}: ${slug}`);
                }
            } catch (err) {
                failed++;
                logs.push(`Error processing ${slug}: ${err.message}`);
            }
        }
