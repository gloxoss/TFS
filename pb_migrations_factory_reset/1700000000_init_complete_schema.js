...
migrate((app) => {
    ...
    const rules = {
        "listRule": "",
        "viewRule": "",
        "createRule": null, // Admin only for now
        "updateRule": null,
        "deleteRule": null,
    };
    ...
    let equipment = new Collection({
        ...
        viewRule: "",
        schema: [
            { name: "name", type: "text" }, // Computed/Fallback
            ...
            // Specs (JSON vs Fields - relying on JSON as per previous files)
            { name: "specs_en", type: "json" },
            { name: "specs_fr", type: "json" },
            { name: "type", type: "text" }, // e.g. Anamorphic, used for filtering
    ...
    let kitItems = new Collection({
        ...
        schema: [
            ...
            { name: "swappable_category", type: "text" } // Could be relation, but keeping simple for now
        ]
    });
    app.save(kitItems);
    ...
    // 6. CARTS
    // -------------------------------------------------------------------------
    // Assuming users collection already exists (system).
    ...
    let carts = new Collection({
        ...
        schema: [
            ...
            { name: "items", type: "json" }, // Storing full cart blobs for speed/flexibility
            { name: "dates_start", type: "date" },
            { name: "dates_end", type: "date" }
        ]
    });

    // Validate rules after schema is defined
    ...
    let services = new Collection({
        ...
        schema: [
            ...
            { name: "icon", type: "text" } // feather icon name
        ]
    });
    app.save(services);

}, (app) => {
    // Down migration: delete all
