...
export default function ServiceHeroSection({ title, description, heroImage, lng, serviceId }: ServiceHeroSectionProps) {
    // Construct image URL
    const getHeroUrl = (image: string) => {
        if (!image) return null
        if (image.startsWith('http') || image.startsWith('/')) return image
        return `${process.env.NEXT_PUBLIC_POCKETBASE_URL || ''}/api/files/services/${serviceId}/${image}`
    }

    const imageUrl = heroImage ? getHeroUrl(heroImage) : 'https://images.unsplash.com/photo-1485846234645-a62644f84728?q=80&w=2000&auto=format&fit=crop'
        ...
        <section className="relative h-screen min-h-[700px] flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-10 container mx-auto px-6 py-32 text-center max-w-5xl">
                ...
                >
                    ...
                    >
                        {lng === 'fr' ? 'Contactez-nous' : 'Talk to Us'}
