...
export function ProductDetailClient({ product, lng }: ProductDetailClientProps) {
  ...
  useEffect(() => {
    if (resolvedKit && !initRef.current) {
      initRef.current = true

      const essentials = resolvedKit.slots
        .filter(s => ESSENTIAL_KEYWORDS.some(k => s.slotName.toLowerCase().includes(k)))
        .map(s => s.slotName)

      // If no essentials match (rendering issue?), default to first 3
      if (essentials.length === 0 && resolvedKit.slots.length > 0) {
        setVisibleSlots(resolvedKit.slots.slice(0, 3).map(s => s.slotName))
      } else {
        setVisibleSlots(essentials)
      }
    }
  }, [resolvedKit])
  ...
  const handleAddToCart = useCallback(async () => {
    ...
    setIsAdding(false)
    openCartDrawer()
  }, [product, quantity, addItem, addToast, openCartDrawer, resolvedKit, kitSelections, useMockKit, mockSelections, clearSelections, selectedVariantOptions, getVariantSelections])
    ...
    return (
      <AnimatePresence mode="wait">
        ...
        >
            ...
            >
              ...
              <motion.div variants={fadeInBlur} className="lg:col-span-2 flex justify-center">
                <ProductImageGallery
                  ...
                  categoryBadge={product.category?.name}
                  compact
                />
              </motion.div>
              ...
              <motion.div variants={fadeInBlur} className="lg:col-span-3 flex flex-col">
                ...
                {product.description && (
                  <div className="text-zinc-400 leading-relaxed mb-6 whitespace-pre-line">
          ...
          {/* CONFIGURABLE ACCESSORY PICKER */}
          {hiddenSlotData.length > 0 && (
            <motion.div
              variants={fadeInBlur}
              initial="visible"
              animate="visible"
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Footer */}
        <div className="p-4 border-t border-zinc-800 bg-zinc-900/50 flex items-center justify-between flex-shrink-0">
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault()
            ...
            className={cn(
              "px-4 py-2 text-sm font-medium transition-colors flex items-center gap-2 rounded-lg",
