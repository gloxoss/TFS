...
export default function QuotePage({ params, searchParams }: PageProps) {
  ...
  const toggleVariant = () => {
    ...
    if (isLegacy) newParams.delete('v')
    else newParams.set('v', 'legacy')
    router.push(`/${lng}/quote?${newParams.toString()}`)
  }
    ...
    return (
      <div className="min-h-screen bg-zinc-950 pt-32 pb-24">
        ...
        <button
          ...
          className="fixed bottom-4 right-4 z-50 bg-zinc-900 text-zinc-500 hover:text-white text-xs px-3 py-1 rounded-full border border-zinc-800 shadow-lg opacity-50 hover:opacity-100 transition-all"
        >
          Switch to Legacy
        </button>
        ...
        <div className="max-w-3xl mx-auto px-4 mb-12 text-center">
          ...
          <h1 className="text-3xl font-bold text-white mb-2">{t(quotePage.title, lng)}</h1>
          <p className="text-zinc-500">Fast, streamlined quote request.</p>
        </div>
